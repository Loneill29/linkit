...
RSpec.describe QuestionsController, type: :controller do
  ...
  describe "PUT update" do
    it "updates question with expected attributes" do
      ...
      new_value = RandomData.random_boolean

      put :update, params: { id: my_question.id, question: { title: new_title, body: new_body, resolved: new_value } }

      updated_question = assigns(:question)
      expect(updated_question.id).to eq(my_question.id)
      expect(updated_question.title).to eq(new_title)
      expect(updated_question.body).to eq(new_body)
      expect(updated_question.resolved).to eq(new_value)
    end
  ...
  describe "DELETE destroy" do
    it "deletes the question" do
      ...
      #make sure the database no longer contains a matching question
